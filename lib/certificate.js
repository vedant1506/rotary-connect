...
function drawLogoPlaceholder(pdf, x, y, size) {
  ...
  pdf.setFontSize(6.5);
  pdf.text('Place in /public/logo.jpeg', x + size / 2, y + size / 2 + 2.5, { align: 'center', maxWidth: size - 4 });
}
...
export async function generateVolunteerCertificate(volunteerName, eventName = 'Scheduled Community Service Drive') {
  ...
  const logoDataUrl = await loadImageDataUrl('/logo.jpeg');
  ...
  pdf.setFont('helvetica', 'bold');
  pdf.setTextColor(247, 168, 27);
  pdf.setFontSize(24);
  pdf.text('ROTARY CLUB VISNAGAR', centerX + 12, 48, { align: 'center' });
  ...
  pdf.setFontSize(10);
  pdf.text('Rotary Club visnagar', pageWidth - 22, pageHeight - 22, { align: 'right' });
