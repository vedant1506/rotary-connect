...
async function extract(pdfPath) {
  ...
  // Split into paragraphs/blocks by two or more newlines
  ...
  for (const block of blocks) {
    // Try to detect a date inside the block
    let foundDate = null;
    ...
    if (foundDate) {
      // Use first line as title, find location line if present
      const lines = block.split(/\n+/).map(l => l.trim()).filter(Boolean);
      const title = lines[0] || block.substring(0, 60);
      let location = '';
      let description = lines.slice(1).join(' ');

      // Try to extract a location keyword
  ...
  // If we found none, try a looser parse: lines that contain the word 'Event' or 'Drive'
  if (events.length === 0) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/\b(Event|Drive|Camp|Camp:)\b/i.test(line)) {
        ...
        const m = next.match(/(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}|\d{4}-\d{2}-\d{2}|${monthNames} \d{1,2},? \d{4})/i);
        const date = m ? m[0] : '';
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: node extract-past-events.js <path-to-pdf>');
    process.exit(1);
  }
  ...
  try {
    const events = await extract(pdfPath);
