...
export default function ParticipantModal({ open, event, onClose }) {
  ...
  async function handleSubmit(submitEvent) {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: 'Registration submitted successfully. We will contact you before the drive!',
      });
      setFormData(initialFormState);
    } catch (submitError) {
      setFeedback({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center px-0 py-0 sm:items-center sm:px-4 sm:py-6">
      <button
        type="button"
        aria-label="Close participant registration modal"
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-t-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20 sm:rounded-3xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-4 py-4 sm:px-6 sm:py-5">
          <div>
            ...
            <h2 className="mt-1 text-lg font-semibold text-slate-950 sm:text-xl">
              Register as Patient for {selectedEventTitle}
            </h2>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="max-h-[80vh] space-y-5 overflow-y-auto px-4 py-4 sm:max-h-[85vh] sm:px-6 sm:py-6">
          ...
          <label className="space-y-2 block">
            <span className="text-sm font-medium text-slate-700">Required Service</span>
            <select
              required
              value={formData.requiredService}
              onChange={(e) => setFormData((current) => ({ ...current, requiredService: e.target.value }))}
              className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:bg-white"
            >
          ...
          <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:items-center sm:justify-end">
            ...
            <button
              ...
              className="rounded-full border border-[#f7a81b] bg-white px-5 py-2.5 text-sm font-semibold text-[#f7a81b] transition-colors hover:bg-[#f7a81b] hover:text-white disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
            >
